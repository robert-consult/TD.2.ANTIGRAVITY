...
export function LiveUpdatesProvider({ children }: { children: ReactNode }) {
  ...
  const runtimeIntervals = useMemo(
    () => resolveRuntimeIntervals(hints, performanceSettings),
    [hints, performanceSettings],
  );

  const { isConnected, sendMessage } = useWebSocket(wsUrl, {
    enabled: isAuthenticated,
    reconnectInterval: runtimeIntervals.ws.reconnectBaseDelayMs,
    reconnectAttempts: runtimeIntervals.ws.reconnectAttempts,
    onMessage: (message) => {
