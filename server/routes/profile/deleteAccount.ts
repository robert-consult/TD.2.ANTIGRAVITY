...
export function registerDeleteAccountRoute(router: Router, deps: ProfileRouterDeps) {
  const { ensureAuth, sessionCookieName } = deps;
  const SESSION_COOKIE_NAME = sessionCookieName;
router.post("/api/profile/account/delete", ensureAuth, async (req: Request, res: Response) => {
  try {
