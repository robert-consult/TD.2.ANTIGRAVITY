...
async function getEnforcementState(db: GriftDb, userId: number): Promise<EnforcementState> {
  const row = await db
    .prepare(
      `
...
export async function maybeApplyAutoEnforcement(
  ...
  const existing = await getEnforcementState(db, userId);
  ...
  if (desired === "FREEZE") {
    ...
    await appendAuditEntry(db, SYSTEM_ADMIN_ID, "ENFORCEMENT_FREEZE", "user", userId, {
      mode: "AUTO",
      scoreCurrent,
      threshold,
      oldStatus,
    ...
    await appendAuditEntry(db, SYSTEM_ADMIN_ID, "ENFORCEMENT_DISABLE", "user", userId, {
      mode: "AUTO",
      scoreCurrent,
      threshold,
      oldStatus,
