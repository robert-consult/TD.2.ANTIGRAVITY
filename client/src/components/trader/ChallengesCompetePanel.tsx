            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {(leaderboardQuery.data?.rows ?? []).map((row: AnyRow) => (
                  <tr key={`${row.rank}-${row.anonId ?? row.userId ?? row.displayName ?? "row"}`} className={`border-b border-neutral-800/90 ${row.isYou ? "bg-neutral-700/30" : ""}`}>
                    <td className="py-2 px-2">#{toInt(row.rank, 0)}</td>
                    <td className="py-2 px-2">{row.displayName || `Trader #${row.anonId ?? row.userId ?? "-"}`}</td>
