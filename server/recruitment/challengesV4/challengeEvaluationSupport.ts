...
export const EVALUATION_RUN_LOCK_KEY = 920_451_172;
export const customRewardRuleCache = new Map<string, ReturnType<typeof parseCustomRewardRules>>();

export function buildEvalRunId(now: number): string {
  return `challenge-eval-${now}-${Math.floor(Math.random() * 1_000_000_000).toString(36)}`;
}

export function withRunDetails(details: unknown, runId: string): Record<string, unknown> {
...
export async function appendChallengeEventWithRun(
...
export function appendChallengeTransitionAudit(input: {
  runId: string;
  userId: number;
  challengeId: number;
  enrollmentId: number;
...
export function roundDeterministic(value: number, decimals = 8): number {
...
export function normalizePrizeDistribution(raw: unknown): Array<{ rank: number; pct: number }> {
...
export async function resolveBadgeByRef(tx: any, badgeRef: unknown): Promise<{ id: number; name: string } | null> {
...
export async function awardBadge(args: {
  userId: number;
  challengeId: number;
  enrollmentId: number;
  badgeRef: unknown;
...
export async function awardSelectionBoost(args: {
  userId: number;
  challengeId: number;
  enrollmentId: number;
  points: number;
...
export async function claimCustomRewardLedger(args: {
  enrollmentId: number;
  challengeId: number;
  userId: number;
...
export async function executeCustomRewardAction(args: {
  enrollment: any;
  challenge: any;
  cfg: SystemChallengeConfig;
  trigger: CustomRewardTrigger;
...
export async function applyCustomRewardsForTrigger(args: {
  cfg: SystemChallengeConfig;
  enrollment: any;
  challenge: any;
  trigger: CustomRewardTrigger;
...
export async function upsertPipelineVisibility(args: {
  userId: number;
...
export function resolvePrizeAwardTiming(challenge: any, cfg: SystemChallengeConfig): "ON_COMPLETE" | "ON_CHALLENGE_END" | "MANUAL" {
...
export function resolveBreachPolicy(challenge: any, cfg: SystemChallengeConfig): "FAIL" | "BREACH_AND_CONTINUE" | "MANUAL_REVIEW" {
...
export function resolvePrizeCandidateMode(challenge: any, cfg: SystemChallengeConfig): "PASSED_ONLY" | "INCLUDE_ACTIVE" {
...
export async function rankPrizeCandidates(
  challengeId: number,
...
export async function recomputePrizeAwards(args: {
  challenge: any;
  cfg: SystemChallengeConfig;
...
export function parseTierRules(raw: unknown): TierRule[] {
  const parsed = parseJsonValue(raw);
...
export function resolveProgressionTierName(args: {
  rules: TierRule[];
...
export async function updateUserProgression(args: {
  userId: number;
...
export async function refreshChallengeLeaderboard(args: {
  challengeId: number;
  maxVisible: number;
  calculatedAt: number;
...
export async function maybeRefreshChallengeLeaderboard(args: {
  challenge: any;
  cfg: SystemChallengeConfig;
  now: number;
  force: boolean;
...
export async function applyCompletionRewards(args: {
  enrollment: any;
  challenge: any;
  stats: PhaseStats;
  cfg: SystemChallengeConfig;
...
export async function notifyPrizeAwardsForChallenge(args: {
  challenge: any;
  cfg: SystemChallengeConfig;
  runId: string;
  newlyAwardedEnrollmentIds: Set<number>;
...
export async function persistPhaseSnapshot(args: {
  enrollmentId: number;
  challengeId: number;
  userId: number;
  phaseNumber: number;
