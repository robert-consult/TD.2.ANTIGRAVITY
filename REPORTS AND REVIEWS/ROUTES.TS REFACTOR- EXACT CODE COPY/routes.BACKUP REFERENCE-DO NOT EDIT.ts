...
import { resolveCaptchaProvider, verifySignupCaptcha } from "./security/captcha";
...
const CURRENT_USER_RECALC_STATE = new Map<number, { lastRunAtMs: number; inFlight: Promise<void> | null; touchedAtMs: number }>();
...
function toUnixMs(value: unknown, fallbackMs = Date.now()): number {
...
function computeEmailGracePeriod(createdAt: unknown, emailVerified: boolean): {
...
async function maybeRecalcAccountForCurrentUser(userId: number): Promise<void> {
  const nowMs = Date.now();
